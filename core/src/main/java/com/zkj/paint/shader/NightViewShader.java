...
public class NightViewShader implements Shader{
    ...
    @Override
    public void create() {
        String nightSkyVertexShader = Gdx.files.internal("shader/night_sky/night_sky.vert").readString();
        String nightSkyFragmentShader = Gdx.files.internal("shader/night_sky/night_sky.frag").readString();

        // 创建着色器程序
        shaderProgram = new ShaderProgram(nightSkyVertexShader, nightSkyFragmentShader);

        // 检查着色器程序是否编译成功
        if (!shaderProgram.isCompiled()) {
            Gdx.app.error("Night Sky Shader", "Shader compilation failed: " + shaderProgram.getLog());
            return;
        }
        // 初始化VBO
        initVBO();
